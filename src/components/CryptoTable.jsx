...
/* ── Animated price: analogue clock-style digit flips ── */
const AnimatedPrice = ({ value, dir, v }) => {
	const animClass = dir ? (dir === 'up' ? 'flip-up' : 'flip-down') : '';
	return (
		<span
			key={dir ? v : 'stable'}
			className={`inline-flex tabular-nums text-base leading-[1em] font-normal text-white ${animClass}`}
		...
		<div className="flex flex-col w-full py-4 border-b border-white/[0.08] transition-colors duration-150 hover:bg-white/[0.04] rounded-lg cursor-pointer">
			<div className="flex items-center justify-between w-full gap-4">
				...
				<div className="flex flex-col items-end px-2 py-0.5">
					<AnimatedPrice value={formatPrice(asset.current_price)} dir={asset.dir} v={asset.v} />
